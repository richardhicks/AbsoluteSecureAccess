...
.GUID a447d12a-8ab9-4365-898c-63c482d8c22a
...
.LICENSEURI https://github.com/richardhicks/AbsoluteSecureAccess/blob/main/LICENSE

.PROJECTURI https://github.com/richardhicks/AbsoluteSecureAccess

.TAGS AbsoluteSecureAccess
...
.SYNOPSIS
    Updates firewall rules for Absolute Secure Access and Insights for Networks infrastructure servers.

.PARAMETER ServerRole
    Use this parameter to define the installed server role. Valid values are SecureAccessServer, InsightsForNetworksServer, and BandwidthTestServer.

.EXAMPLE
    .\Update-AbsoluteSecureAccessFirewallRule.ps1 -ServerRole SecureAccessServer

    Updates the firewall rules for the SecureAccessServer role.

.EXAMPLE
    .\Update-AbsoluteSecureAccessFirewallRule.ps1 -ServerRole InsightsForNetworksServer

    Updates the firewall rules for the InsightsForNetworksServer role.

.EXAMPLE
    .\Update-AbsoluteSecureAccessFirewallRule.ps1 -ServerRole BandwidthTestServer

    Updates the firewall rules for the BandwidthTestServer role.

.DESCRIPTION
    The Absolute Secure Access and Insights for Networks installer will, by default, create firewall rules for the required infrastructure servers. However, these rules are not created in a firewall rule group and are not easily identifiable. This script will create a firewall rule group named "Absolute Secure Access" and add the required firewall rules to that group. This makes it easier to identify the required firewall rules and manage them as a group.

.LINK
    https://github.com/richardhicks/AbsoluteSecureAccess/blob/main/Update-AbsoluteSecureAccessFirewallRule.ps1

.LINK
    https://directaccess.richardhicks.com/absolute-secure-access/
...
Param(
    ...
    [Parameter(Mandatory)]
    [ValidateSet('SecureAccessServer', 'InsightsForNetworksServer', 'BandwidthTestServer')]
    [string]$ServerRole

)

# Determine if the server is domain-joined
$DomainJoined = (Get-CimInstance -ClassName Win32_ComputerSystem).PartOfDomain

If ($DomainJoined) {

    Write-Verbose "Server $env:computername is domain-joined."

}

# Clean up default Absolute Secure Access firewall rules and add inbound rule for Absolute Secure Access connections
If ($ServerRole -eq 'SecureAccessServer') {

    Write-Verbose "Server role $ServerRole selected."
    $Rules = Get-NetFirewallRule | Where-Object DisplayName -Match 'Secure Access' -ErrorAction SilentlyContinue

    If ($Null -eq $Rules) {

        Write-Warning 'No Absolute Secure Access firewall rules found. Ensure the selected server role is correct and that the server software is installed.'
        Return
    ...
    ForEach ($Rule in $Rules) {
        ...
        Write-Verbose "Updating firewall rule $($Rule.DisplayName)..."
        $Rule.Group = 'Absolute Secure Access'

        # Restrict firewall rule to the Windows domain profile if the server is domain-joined
        If ($DomainJoined) {

            Write-Verbose "Restricting firewall rule $($Rule.DisplayName) to domain profile..."
            $Rule.Profile = 'Domain'
    ...
    # Add firewall rule to allow inbound Absolute Secure Access connections on UDP port 5008
    Write-Verbose 'Adding firewall rule to allow inbound Absolute Secure Access connections on UDP port 5008...'
    $Rule = New-NetFirewallRule -DisplayName 'Secure Access Connections' -Group 'Absolute Secure Access' -Profile Any -Direction Inbound -Action Allow -Protocol UDP -LocalPort 5008 -ErrorAction SilentlyContinue
    $Rule | Set-NetFirewallRule

}

# Clean up default Absolute Secure Access Insights for Networks firewall rules
If ($ServerRole -eq 'InsightsForNetworksServer') {

    Write-Verbose "Server role $ServerRole selected."
    $Rules = Get-NetFirewallRule | Where-Object DisplayName -Match 'Mobile IQ' -ErrorAction SilentlyContinue

    If ($Null -eq $Rules) {

        Write-Warning 'No Insights for Networks firewall rules found. Ensure the selected server role is correct and that the server software is installed.'
        Return
...
# Clean up default Absolute Secure Access bandwidth test server firewall rule
If ($ServerRole -eq 'BandwidthTestServer') {

    Write-Verbose "Server role $ServerRole selected."
    $Rule = Get-NetFirewallRule -DisplayName 'Secure Access Bandwidth Server' -ErrorAction SilentlyContinue

    If ($Null -eq $Rule) {

        Write-Warning 'No Absolute Secure Access Bandwidth Test Server firewall rules found. Ensure the server role is correct and that the server software is installed.'
        Return
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUM8r/kzRArb6lXD6J8TztuVCM
# PRaggiDDMIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# MRYEFEKGZt4E9ocClAc+ASV9WGrm3yxfMA0GCSqGSIb3DQEBAQUABIIBgDeiLSty
# CvvMzoOyL5qDdOTGGNQqPgAuK8oU1VZ1BRliLNKXOD409QpV2CHTa9iMaHKPvdJC
# yGpadehygaGnjPkPBFag8o34ssu4kF5gbfpOH+RC81XYhs1VsdMTJqD2nZkxOVc/
# VkkZ3k1fOnt70MaMud+L1gzhkakzoTVegVcvokXO7AepXZhrdj5x+04NL+zGclAx
# 7NkV1CsgWfQqJ/DdJf2rIymTKrynlcL9Eio63BO/T03GLOGmuj5p4etGyXJERFBX
# zL0HQKZN2d2u3b3OgYb5NZY8z1sRgdNnMI6kKIs2U6vKhouNJvK3UU5IQJpuq+3G
# IZF3WFz7ZUjh4/udUlwmljK2GQyv0o7wr+iH3HQRhvuWXxk6XaATK6AsyQE0lman
# iqc3PskoUd8gf6rLg+Hz5hjXjZ404FpHTyZHQW3PGb76bGUE42YsuEAjIbAuX92i
# xOb4EVmDPUqf1ijeNt3OhjtSaQxohv62rHwc571q75pOn05BXumewmNgU6GCAyAw
...
# CSqGSIb3DQEJBTEPFw0yMzEyMTIxNzQ5NDNaMC8GCSqGSIb3DQEJBDEiBCBbPJ1+
# MN+qgbMCeZACL2QeD53wMsFTWRfgdKBBWTK3/jANBgkqhkiG9w0BAQEFAASCAgAv
# do6TavQ6foCLENvH5SD888lJTklIbp8VC4YmanPv0RE9RFXGsjsQWsjUFgnRbkMF
# T4ipdlQ2GBzbgg//D693qyfI3DsYfMcFiu58cQdw/EGj3XTVywnOX+hBAEYAEk9n
# ASca6Q+K55pYjyA1ZfPSXBqRdGj5VGUE/8AIxbt3ZTWEJTo3lnpbp+XjsNX2+s5t
# NRcfDhLkJ6uV/UyX38r2IC6Ese7htYGL42VzhZHerZfysIYZLleIu4SiDpj3dmKr
# MaP1ydIJY2KHiCU/2mU1gj6RymNfIC4Ny+Hjj8KD1oa8SAjNA6AW/cJbhTh3XpYh
# RvWq1WpU7zzuCDmV9HbfoaggdD8T96BjNHPD+8ZG+bd9iuasb/cVtBwHMJwW3nAf
# nPB2QroBajY8iQs1A350Tif3GtMFVTMsJisH7lXKbmGfc+mDZFN4AeKFgLcXD7th
# eSJVdTFBJ+5Bl3GAaiNn8aTF4AWHz27uB5ViBsxUXwzjZYG00Moyz15D7qPWt5D5
# lkup981YHQKqF7XDJZKJrv4IREm7Jy4TAlV3OxrW1FG9yBix+eB2t+oeIAuUwN7r
# wLmVsMMvKYYJzjMANNUYDdrjrSouvLUxUMWVoUB8/hWHxfRMuprdLuGdrCVUdNNk
# KlKTufR4/lP1YXG/gjfJbTSfw6qLtoMW6/sKODFARA==
